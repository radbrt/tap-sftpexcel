...
from tap_sftpexcel import decrypt
    ...
    def get_file_handle(self, f, decryption_configs=None):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if decryption_configs:
                ...
                self.sftp.get(sftp_file_path, local_path)
                df = pd.read_excel(local_path, header=1, dtype=str)

                csv_path = f"{local_path}.csv"
                df.to_csv(f"{local_path}.csv", index=False, quoting=csv.QUOTE_ALL, sep='|')
